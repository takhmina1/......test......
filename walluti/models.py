...
# # Модель для хранения данных о фиатных валютах
...
#     fiatMaxLimit = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)  # Максимальный лимит для фиатных операций
...
# # Модель для хранения изображений фиатных валют
# # class FiatCurrencyImage(models.Model):
# #     currency = models.ForeignKey(FiatCurrency, related_name='images', on_delete=models.CASCADE)  # Ссылка на фиатную валюту
# #     image_url = models.URLField()  # URL-адрес изображения

# #     def __str__(self):
# #         return f"Изображение для {self.currency.assetName}"  # Возвращает строковое представление объекта

# #     class Meta:
# #         verbose_name = "Изображение фиатной валюты"  # Название модели в единственном числе
# #         verbose_name_plural = "Изображения фиатных валют"  # Название модели во множественном числе



# """ криптовалюты  """

# # Модель Coin представляет криптовалюту с основными характеристиками.
...
# # Модель Network представляет сеть, доступную для криптовалюты.
...
#     # contract_address = models.CharField(max_length=100, blank=True, null=True)  # Адрес контракта (если есть)
#     # contract_address_url = models.URLField(max_length=200, blank=True, null=True)  # Ссылка на контракт в исследователе блоков
#     # explorer_contract = models.URLField(max_length=200, blank=True, null=True)  # Ссылка на контракт в исследователе блоков

#     def __str__(self):
#         return f"{self.coin.name} - {self.name}"  # Возвращает строковое представление сети

# # Модель Provider связывает криптовалюту с информацией о провайдере услуг.
# class Provider(models.Model):
#     coin = models.OneToOneField(Coin, on_delete=models.CASCADE, primary_key=True)  # Ссылка на криптовалюту
#     # Предполагается, что 'provider_info' может храниться как JSONField в PostgreSQL.
#     # Для простоты здесь используется только ключевое поле ForeignKey.

#     def __str__(self):
#         return self.coin.name  # Возвращает название криптовалюты при обращении к объекту
...
# #     class Meta:
# #         verbose_name = "Изображение Currency валюты"  # Название модели в единственном числе
# #         verbose_name_plural = "Изображения Currency валют"  # Название модели во множественном числе
